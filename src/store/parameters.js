...
export const parameters = {
  ...
  getters: {
    ...
    autoCalcPeriodicPoling: (state) =>
      state.spdConfig.periodic_poling_enabled && state.autoCalcPeriodicPoling,
    ...
    apodizationPointsLength: (state) =>
      state.spdConfig.apodization_points.length,
    ...
    integratorMethod: (state) => state.spdConfig.integrator.method,
    integratorMethods: () => IntegratorMethods.slice(0),
    ...
    minPumpWaistSize: (state, getters) =>
      (MUCH_LARGER * (1e-9 / 1e-6) * getters.pumpWavelength) /
      getters.refractiveIndices.np,
    minSignalWaistSize: (state, getters) =>
      (MUCH_LARGER * (1e-9 / 1e-6) * getters.signalWavelength) /
      getters.refractiveIndices.ns,
    minIdlerWaistSize: (state, getters) =>
      (MUCH_LARGER * (1e-9 / 1e-6) * getters.idlerWavelength) /
      getters.refractiveIndices.ni,
  },
  actions: {
