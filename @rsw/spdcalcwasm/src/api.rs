...
impl SPDConfig {
  pub fn from_spdc(spdc: spdcalc::SPDC) -> Self {
    use spdcalc::{ApodizationConfig, PeriodicPolingConfig};
    ...
    SPDConfig {
      ...
      signal_waist_position: match config.signal.waist_position_um {
        spdcalc::AutoCalcParam::Param(p) => p,
        spdcalc::AutoCalcParam::Auto(_) => {
          panic!("signal waist position must be specified")
        }
      }, // microns
      ...
      apodization_fwhm: match &config.periodic_poling {
        PeriodicPolingConfig::Config {
          apodization: ApodizationConfig::Gaussian { fwhm_um },
          ..
        } => *fwhm_um,
        _ => 0.0,
      },
      apodization_param: match &config.periodic_poling {
        PeriodicPolingConfig::Config {
          apodization: ApodizationConfig::Interpolate(_),
          ..
        } => 1.0,
        PeriodicPolingConfig::Config {
          apodization: ApodizationConfig::Bartlett(p),
          ..
        } => *p,
        PeriodicPolingConfig::Config {
          apodization: ApodizationConfig::Blackman(p),
          ..
        } => *p,
        PeriodicPolingConfig::Config {
          apodization: ApodizationConfig::Connes(p),
          ..
        } => *p,
        PeriodicPolingConfig::Config {
          apodization: ApodizationConfig::Cosine(p),
          ..
        } => *p,
        PeriodicPolingConfig::Config {
          apodization: ApodizationConfig::Hamming(p),
          ..
        } => *p,
        PeriodicPolingConfig::Config {
          apodization: ApodizationConfig::Welch(p),
          ..
        } => *p,
        _ => 1.0,
      },
      apodization_points: match config.periodic_poling {
        PeriodicPolingConfig::Config {
          apodization: ApodizationConfig::Interpolate(points),
          ..
        } => points,
        _ => vec![],
      },
...
impl From<SPDConfig> for spdcalc::SPDCConfig {
  fn from(cfg: SPDConfig) -> Self {
    use spdcalc::{
      ApodizationConfig, AutoCalcParam, CrystalConfig, PeriodicPolingConfig, PumpConfig,
      SignalConfig,
    };
    use std::str::FromStr;
    ...
    let periodic_poling = if cfg.periodic_poling_enabled {
      let apodization = if cfg.apodization_enabled {
        match cfg.apodization_type {
          ApodizationType::Gaussian => ApodizationConfig::Gaussian {
            fwhm_um: cfg.apodization_fwhm,
          },
    ...
    // idler waist and waist position are overridden in unwrap_cfg
...
impl JointSpectrum {
  fn from_spdc<
    T: spdcalc::jsa::IntoSignalIdlerIterator + Copy + Into<spdcalc::jsa::FrequencySpace>,
  >(
    spdc: &spdcalc::SPDC,
    range: T,
    integrator: Integrator,
  ) -> Self {
    let spectrum = spdc.joint_spectrum(integrator);
    ...
    let (amplitudes, phases) = spectrum
      .jsa_normalized_range(range)
      .iter()
      .map(|c| c.to_polar())
      .unzip();
...
fn unwrap_cfg(cfg: JsValue) -> Result<(spdcalc::SPDC, Integrator), APIError> {
  ...
  let integrator = spd_config.integrator;
...
pub fn get_json(spd_config_raw: JsValue) -> Result<JsValue, JsError> {
  ...
  let json = serde_json::to_string_pretty(&spdcalc::SPDCConfig::from(config))
    .map_err(|_| APIError("Problem converting json".into()))?;
  Ok(json.into())
}
...
pub fn get_params_from_json(json: String) -> Result<JsValue, JsError> {
  let config: spdcalc::SPDCConfig =
    serde_json::from_str(&json).map_err(|_| APIError("Problem converting json".into()))?;
...
#[wasm_bindgen]
pub fn get_joint_spectrum_sum_diff(
  spd_config_raw: JsValue,
  integration_config: IntegrationConfig,
) -> Result<JointSpectrum, JsError> {
  let (spdc, integrator) = unwrap_cfg(spd_config_raw)?;
...
pub fn get_waist_positions(spd_config_raw: JsValue) -> Result<Vec<f64>, JsError> {
  ...
  let z0s = spdc
    .crystal_setup
    .optimal_waist_position(spdc.signal.vacuum_wavelength(), spdc.signal.polarization());
  let z0i = spdc
    .crystal_setup
    .optimal_waist_position(spdc.idler.vacuum_wavelength(), spdc.idler.polarization());
  Ok(vec![*(z0s / (MICRO * M)), *(z0i / (MICRO * M))])
}
...
#[wasm_bindgen]
pub fn get_hom_series_data(
  spd_config_raw: JsValue,
  integration_config: IntegrationConfig,
  time_steps_femto_raw: JsValue,
) -> Result<Vec<f64>, JsError> {
  let (spdc, integrator) = unwrap_cfg(spd_config_raw)?;
  let time_steps = parse_time_steps(time_steps_femto_raw, FEMTO)?;
  let ranges = spdcalc::jsa::WavelengthSpace::from(Steps2D::from(integration_config));
  Ok(spdc.hom_rate_series(time_steps, ranges, integrator))
}

#[wasm_bindgen]
pub fn get_hom_visibility(
  spd_config_raw: JsValue,
  integration_config: IntegrationConfig,
) -> Result<Vec<f64>, JsError> {
  let (spdc, integrator) = unwrap_cfg(spd_config_raw.clone())?;
  let ranges = spdcalc::jsa::WavelengthSpace::from(Steps2D::from(integration_config));
  let (delta_t, vis) = spdc.hom_visibility(ranges, integrator);
  Ok(vec![*(delta_t / S), vis])
}

#[wasm_bindgen]
pub fn get_hom_two_source_series_data(
  spd_config_raw: JsValue,
  integration_config: IntegrationConfig,
  time_steps_femto_raw: JsValue,
  ...
  let time_steps = parse_time_steps(time_steps_femto_raw, FEMTO)?;
  let ranges = spdcalc::jsa::WavelengthSpace::from(Steps2D::from(integration_config));
  let data = spdc.hom_two_source_rate_series(time_steps, ranges, integrator);
  Ok(serde_wasm_bindgen::to_value(&data)?)
}

#[wasm_bindgen]
pub fn get_hom_two_source_visibility(
  spd_config_raw: JsValue,
  integration_config: IntegrationConfig,
) -> Result<JsValue, JsError> {
  let (spdc, integrator) = unwrap_cfg(spd_config_raw.clone())?;
  let ranges = spdcalc::jsa::WavelengthSpace::from(Steps2D::from(integration_config));
  let data = spdc.hom_two_source_visibilities(ranges, integrator);
  Ok(serde_wasm_bindgen::to_value(&data)?)
}

#[wasm_bindgen]
pub fn get_heralding_results(
  spd_config_raw: JsValue,
  integration_config: IntegrationConfig,
) -> Result<JsValue, JsError> {
  let (spdc, integrator) = unwrap_cfg(spd_config_raw)?;
  let ranges = spdcalc::jsa::WavelengthSpace::from(Steps2D::from(integration_config));
  let data = spdc.efficiencies(ranges, integrator);
  Ok(serde_wasm_bindgen::to_value(&data)?)
}
...
pub fn get_heralding_results_vs_waist(
  ...
  let ranges = spdcalc::jsa::WavelengthSpace::from(Steps2D::from(integration_config));
  let waist_steps_microns: Steps<f64> =
    serde_wasm_bindgen::from_value(waist_steps_microns_raw).map_err(APIError::from)?;
  let data: Vec<spdcalc::Efficiencies> = waist_steps_microns
    .into_iter()
    .map(move |w| {
      spdc.signal.set_waist(w * MICRO * M);
      spdc.idler.set_waist(w * MICRO * M);
      spdc.efficiencies(ranges, integrator)
    })
    .collect();
  Ok(serde_wasm_bindgen::to_value(&data)?)
}
...
pub fn get_heralding_results_vs_signal_theta(
  ...
  let (mut spdc, integrator) = unwrap_cfg(spd_config_raw)?;
  let ranges = spdcalc::jsa::WavelengthSpace::from(Steps2D::from(integration_config));
  let theta_steps_deg: Steps<f64> =
    serde_wasm_bindgen::from_value(theta_steps_deg_raw).map_err(APIError::from)?;

  let ret: Vec<spdcalc::Efficiencies> = theta_steps_deg
    .into_iter()
    .map(move |theta| {
      let theta = theta * DEG;
...
pub fn get_heralding_results_signal_vs_idler_waists(
  ...
  let ret: Vec<spdcalc::Efficiencies> = waists
    .into_iter()
    .map(move |(ws, wi)| {
...
pub fn get_jsi_csi(
  ...
  let max = coincidences
    .iter()
    .chain(signal_singles.iter())
    .chain(idler_singles.iter())
...
pub fn get_schmidt_pump_bw_vs_crystal_length(
  ...
  let results: Result<Vec<f64>, _> = steps
    .into_iter()
    .map(|(len, pbw)| {
      spdc.crystal_setup.length = len * M;
      spdc.pump_bandwidth = pbw * M;
      spdc.joint_spectrum(integrator).schmidt_number(ranges)
    })
    .collect();
...
pub fn get_schmidt_signal_vs_pump_waist(
  ...
  let ranges = spdcalc::jsa::WavelengthSpace::from(Steps2D::from(integration_config));
  let waists: Steps2D<Meter<f64>> = waist_ranges.into();
  let results: Result<Vec<f64>, _> = waists
    .into_iter()
    .map(|(wp, wsi)| {
      spdc.pump.set_waist(wp);
      spdc.signal.set_waist(wsi);
...
pub fn get_hom_visibility_signal_vs_pump_waist(
  ...
  let ranges = spdcalc::jsa::WavelengthSpace::from(Steps2D::from(integration_config));
  let waists: Steps2D<Meter<f64>> = waist_ranges.into();
  let results: Vec<f64> = waists
    .into_iter()
    .map(move |(wp, wsi)| {
      spdc.pump.set_waist(wp);
      spdc.signal.set_waist(wsi);
      spdc.idler.set_waist(wsi);
      spdc.hom_visibility(ranges, integrator).1
    })
    .collect();
...
pub fn delta_k_vs_crystal_theta(spd_config_raw: JsValue) -> Result<Vec<f64>, JsError> {
  ...
  let ret = spdcalc::utils::Steps(mid - dx, mid + dx, 1000)
    ...
    .map(|theta| {
      ...
      (spdc.delta_k(spdc.signal.frequency(), spdc.idler.frequency()) / spdcalc::Wavenumber::new(1.))
        .z
        .abs()
    })
    .collect();
  ...
  let ret = spdcalc::utils::Steps(mid - dx, mid + dx, 1000)
    ...
    .map(|theta| {
      ...
      *(spdc
        ...
        .jsi(spdc.signal.frequency(), spdc.idler.frequency())
        / spdcalc::JSIUnits::new(1.0))
    })
    .collect();
  Ok(ret)
}
...
pub fn delta_k_vs_pp(spd_config_raw: JsValue) -> Result<Vec<f64>, JsError> {
  ...
  let dx = mid * 0.01;
  let ret: Result<Vec<f64>, APIError> = spdcalc::utils::Steps(mid - dx, mid + dx, 1000)
    .into_iter()
    .map(|period| {
      spdc.pp = spdcalc::PeriodicPoling::new(-period, Apodization::Off);
      spdc.assign_optimum_idler()?;
      Ok(
        (spdc.delta_k(spdc.signal.frequency(), spdc.idler.frequency())
          / spdcalc::Wavenumber::new(1.))
        .z
        .abs(),
      )
    })
...
#[wasm_bindgen]
pub fn center_jsi_vs_pp(spd_config_raw: JsValue) -> Result<Vec<f64>, JsError> {
  let (mut spdc, _integrator) = unwrap_cfg(spd_config_raw)?;
  use core::f64::consts::FRAC_PI_2;
  spdc.crystal_setup.theta = FRAC_PI_2 * RAD;
  let mid = match spdc.pp {
...
pub fn poling_domains(spd_config_raw: JsValue) -> Result<Vec<f64>, JsError> {
  ...
  let ret = spdc
    .pp
    .poling_domains(spdc.crystal_setup.length)
    .iter()
    .flat_map(|z| [z.0, z.1])
    .collect();
  Ok(ret)
...
pub fn pm_curve(
  ...
  let iter = spdcalc::SPDCIter::try_new(spdc, prop1, prop2, steps)
    .map_err(|e| JsError::from(APIError(e.to_string())))?;

  Ok(iter.jsi_values(integrator))
}
